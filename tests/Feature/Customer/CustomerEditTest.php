...
it('顧客編集画面のポリシーテスト', function () {
    ...
    // GET /customers/{id}/edit（別ユーザーで作成した顧客）にアクセス
    $response = $this->get("/customers/{$customer->id}/edit");
...
it('顧客更新処理のポリシーテスト', function () {
    ...
    // PATCH /customers/{id}（ログインユーザー以外のユーザーで作成した顧客）にアクセス
    $response = $this->patch("/customers/{$customer->id}", $data);
