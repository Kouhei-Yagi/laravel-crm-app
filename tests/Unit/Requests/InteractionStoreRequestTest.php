...
it('type が許可された値の場合はバリデーションを通過する', function ($validType) {
    // 任意の顧客を作成
    $customer = Customer::factory()->create();
    ...
    // バリデーションを通過することを確認
    expect($validator->fails())->toBeFalse();
    expect($validator->errors()->has('type'))->toBeFalse();
...
it('type が許可されていない値の場合はバリデーションエラーになる', function () {
    // 任意の顧客を作成
    $customer = Customer::factory()->create();
...
it('project_id が Projects テーブルに存在する ID である場合はバリデーションを通過する', function () {
    // 任意の顧客を作成
    $customer = Customer::factory()->create();
    ...
    // バリデーションを通過することを確認
    expect($validator->fails())->toBeFalse();
    expect($validator->errors()->has('project_id'))->toBeFalse();
});

it('project_id が Projects テーブルに存在しない ID である場合はバリデーションエラーになる', function () {
    // 任意の顧客を作成
    $customer = Customer::factory()->create();
    ...
    // バリデーションを通過することを確認
    expect($validator->fails())->toBeTrue();
    expect($validator->errors()->has('project_id'))->toBeTrue();
});

it('customer_id が customers テーブルに存在する ID である場合はバリデーションを通過する', function () {
    // 任意の顧客を作成
    $customer = Customer::factory()->create();
    ...
    // バリデーションを通過することを確認
    expect($validator->fails())->toBeFalse();
    expect($validator->errors()->has('customer_id'))->toBeFalse();
});

it('customer_id が customers テーブルに存在しない ID である場合はバリデーションエラーになる', function () {
    // 送信するデータ（対応履歴作成フォームに入力されたデータ）を作成
    $data = [
        'interacted_at' => '2026-01-01T12:00',
        'type' => 'phone',
