...
- アプリケーションの全体像を短時間で把握できる README に改善するため
- 設計思想・技術的な工夫・学んだことを整理し、ポートフォリオとしての完成度を高めるため
- 画面キャプチャや ER 図を整備し、GitHub 上で視覚的に理解しやすい構成にするため

### ブランチ名：**docs/update-readme**

### 実装日：2026-06-03 ~ 2026-06-07
...
- `docs/images/*`（新規）
- `.env.example`（変更）
- `docs/er-diagram.mmd`（変更）

### 実装内容

- README 全体の構成を見直し、章立てを整理
- 画面キャプチャを新規作成し、19枚を追加
- `.env.example`を日本語環境・SQLite 用に調整
- ER 図を最新のテーブル定義に合わせて更新

### 実装手順

1. README 全体の構成を見直し、章の順序を整理
2. アプリ概要・目的を読みやすく修正
3. 画面キャプチャを作成し、画像ファイルを追加
4. 主な機能一覧を整理し、粒度を統一
5. 使用技術（バックエンド / フロント / テスト）を整理
6. セットアップ手順を見直し、コードブロックを統一
7. ER 図を最新のテーブル定義に合わせて修正
8. 設計思想の文体・構成を統一
9. 技術的な工夫の粒度を揃え、読みやすく調整
10. 「このアプリケーションで学んだこと」を理解ベースで整理
11. 開発プロセスの章を整理し、実際の流れに合わせて記述
12. 使用環境・ディレクトリ構成を確認し、必要箇所を修正
13. Git 運用ルール（GitHub Flow）を整理
14. 今後の改善予定を整理し、粒度を統一
...
- 画像キャプチャの順序・内容の整合性
- ER 図とマイグレーションの整合性
- `.env.example`の設定が README のセットアップ手順と一致しているか
- 「設計思想」「技術的な工夫」「学んだこと」の粒度が揃っているか

### 気づき・課題

- README の文章量が多いため、構成と文体の統一が重要であると実感した
- 画面キャプチャを追加することで、アプリの全体像が伝わりやすくなった
- ER 図はマイグレーション変更時に随時更新する必要があるため、今後も運用ルールとして継続する
