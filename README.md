...
- JOIN を用いた関連データの表示
- 顧客詳細ページで顧客に紐づく案件一覧・対応履歴一覧を表示（未対応）
- 案件詳細ページで案件に紐づく対応履歴一覧を表示（未対応）※任意
- 案件数や最終対応日の集計（未対応）※任意

### 業務システム向け機能

- SoftDeletes（論理削除）
- 権限管理（Policy）による編集・削除制御（未対応）
- CSV エクスポート（顧客一覧）（未対応）
- 更新履歴（Activity Log）（未対応）※任意

### 外部キー制約の方針（概要）

- 顧客削除で案件や対応履歴が消えないよう、restrictOnDelete() を採用
- 案件削除後も対応履歴を残すため、interactions.project_id のみ nullOnDelete()
- 担当者（assigned_user_id）削除でデータが消えるのを防ぐため、すべて restrictOnDelete()
- 実務の CRM では「データの整合性を保つ」ために必須となる設計方針

### 担当者（assigned_user_id）必須の理由

- データの責任範囲を明確にするため
- 担当者不明の顧客・案件・履歴を防ぐため
- 実務の CRM では「誰が対応したか」を必ず記録する
- 担当者削除でデータが消えないよう、外部キーは restrictOnDelete() を採用

### UI / UX

- Tailwind による一覧テーブルの整形
- ステータスバッジ（顧客 / 案件 / 対応履歴）（未対応）
- モーダルでの顧客追加（未対応）※任意
- エラーページのカスタマイズ（未対応）※任意
...
11. ソート処理の共通化（完了）
12. 範囲入れ替え処理の共通化（完了）
13. バリデーションのフォームリクエスト化（完了）
14. Blade コンポーネント化（完了）
15. 顧客詳細ページの強化（案件一覧・対応履歴一覧の表示）
16. 権限管理（Policy）
17. CSV エクスポート
18. Feature Test の追加
19. UI/UX 改善
